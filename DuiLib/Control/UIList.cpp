...
namespace DuiLib {
	...
	CListUI::CListUI() : m_pCallback(NULL), m_bScrollSelect(false), m_iCurSel(-1), m_iExpandedItem(-1), m_bMultiSel(false), m_bEnsureVisible(true)
	{
	...
	bool CListUI::AddAt(CControlUI* pControl, int iIndex)
	{
		...
		for (int i = 0; i < m_aSelItems.GetSize(); ++i) {
			int iIdx = (int)m_aSelItems.GetAt(i);
			if (iIdx >= iIndex)
			{
				m_aSelItems.SetAt(i, (LPVOID)(iIdx + 1));
			}
		}
	...
	void CListUI::SelectAllItems()
	{
		...
		m_bEnsureVisible = bTmp;
	}

	bool CListUI::SelectItems(int nStartIndex, int nEndIndex)
	{
		if (nStartIndex < 0 || nStartIndex > nEndIndex || nEndIndex >= GetCount())
		{
			return false;
		}
		// 取消所有选择项
	...
	{
		if( m_iCurSel < 0 || !m_bEnsureVisible ) return;
