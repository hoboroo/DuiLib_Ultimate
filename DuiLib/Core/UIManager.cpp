...
#import "c:\\windows\\system32\\winhttpcom.dll"
...
namespace DuiLib {
	...
	CDuiString CPaintManagerUI::m_pStrProxy;
	...
	void CPaintManagerUI::SetProxy(LPCTSTR pstrProxy)
	{
		if (pstrProxy)
		{
			m_pStrProxy = pstrProxy;
		}
		else
		{
			m_pStrProxy.Empty();
		...
		case WM_MOUSEHOVER:
			{
				...
				if( m_hwndTooltip == NULL ) {
					...
					::SendMessage(m_hwndTooltip, TTM_SETDELAYTIME, TTDT_INITIAL, pHover->GetToolTipDelayTime());
	...
	const TImageInfo* CPaintManagerUI::GetImageEx(LPCTSTR bitmap, LPCTSTR type, DWORD mask, bool bUseHSL, HINSTANCE instance)
	{
		...
		if (!data) {
			if (!_tcsnicmp(bitmap, _T("http://"), 7) || !_tcsnicmp(bitmap, _T("https://"), 8))
			{
				data = AddWebImage(bitmap, mask, bUseHSL, instance);
			}
			else if (!_tcsnicmp(bitmap, _T("data:image/"), 11) && _tcsstr(bitmap, _T(";base64,")))
			{
				data = AddBase64Image(bitmap, mask, bUseHSL, instance);
			}
			else if (AddImage(bitmap, type, mask, bUseHSL, false, instance)) {
	...
	const TImageInfo* CPaintManagerUI::AddWebImage(LPCTSTR bitmap, DWORD mask /*= 0*/, bool bUseHSL /*= false*/, HINSTANCE instance /*= NULL*/)
	{
		TImageInfo* data = NULL;
		static std::map<CDuiString, TImageInfo*> webImageHash;
		static std::mutex muWebImage;
		static std::map<CDuiString, int> requestCount;

		bool bGet = true;
		muWebImage.lock();
		for (auto it = webImageHash.begin(); it != webImageHash.end();)
		{
			if (it->first == bitmap)
			{
				bGet = false;
			...
			if (it->second)
			{
				if (!m_SharedResInfo.m_ImageHash.Insert(it->first.GetData(), it->second)) {
					CRenderEngine::FreeImage(it->second);
				}
				it = webImageHash.erase(it);
			}
			else
			{
				it++;
		...
		if (bGet && !data)
		{
			webImageHash[bitmap] = NULL;
		}
		muWebImage.unlock();
		if (data || !bGet || requestCount[bitmap] > 10)
		{
			return data;
		}
		...
		std::thread([](HWND hWnd, std::map<CDuiString, TImageInfo*>& webImageHash, std::mutex& mu, CDuiString strName, DWORD mask, bool bUseHSL) {
			CoInitialize(NULL);
			TImageInfo* data = NULL;
			WinHttp::IWinHttpRequestPtr ptrHttp = NULL;
			do
			{
				try
				{
					ptrHttp.CreateInstance(__uuidof(WinHttp::WinHttpRequest));
					ptrHttp->Option[WinHttp::WinHttpRequestOption_EnableRedirects] = VARIANT_TRUE;
					ptrHttp->Option[WinHttp::WinHttpRequestOption_SslErrorIgnoreFlags] = WinHttp::SslErrorFlag_Ignore_All;
					if (!m_pStrProxy.IsEmpty())
					{
						ptrHttp->SetProxy(2, _bstr_t(m_pStrProxy.GetData()));
					}
					ptrHttp->Open("GET", _bstr_t(strName.GetData()));
					ptrHttp->Send();
					_variant_t varRspBody = ptrHttp->GetResponseBody();
					if (varRspBody.vt == VT_EMPTY)
					{
						break;
					}
					ULONG dataLen = varRspBody.parray->rgsabound[0].cElements;
					char* pContentBuffer = (char*)varRspBody.parray->pvData;
					if (!dataLen || !pContentBuffer)
					{
						break;
					}
					int x, y, n;
					LPBYTE pImage = pImage = stbi_load_from_memory((LPBYTE)pContentBuffer, dataLen, &x, &y, &n, 4);
					if (!pImage)
					{
						break;
					}
			...
			if (data)
			{
				TImageInfo* pOld = webImageHash[strName.GetData()];
				if (pOld)
				{
					CRenderEngine::FreeImage(pOld);
				}
				webImageHash[strName.GetData()] = data;
				if (hWnd)
				{
					::PostMessage(hWnd, UIMSG_UPDATE, 0, 0);
				}
			}
			else
			{
				webImageHash.erase(strName);
			}
			muWebImage.unlock();
			CoUninitialize();
		}, m_hWndPaint, std::ref(webImageHash), std::ref(muWebImage), bitmap, mask, bUseHSL).detach();

		return NULL;
	}

	const TImageInfo* CPaintManagerUI::AddBase64Image(LPCTSTR bitmap, DWORD mask /*= 0*/, bool bUseHSL /*= false*/, HINSTANCE instance /*= NULL*/)
	{
		TImageInfo* data = NULL;
		...
		std::wstring tmp(bitmap);
		std::string bmp(tmp.begin(), tmp.end());
		auto idx = bmp.find(";base64,");
		if (idx == string::npos)
		{
			return NULL;
		}
		bmp = bmp.substr(idx + 8);
		std::string src;
		if (B64::Decode(bmp, &src) == false || src.empty())
		{
			return NULL;
		}
		int x, y, n;
		LPBYTE pImage = pImage = stbi_load_from_memory((LPBYTE)src.data(), src.size(), &x, &y, &n, 4);
		if (!pImage)
		{
			return NULL;
		}
