...
namespace DuiLib
{
	...
	{
		CTextUI* pTextTitle = (CTextUI*)m_pm.FindControl(_T("title"));
		if (pTextTitle && pstrTitle)
		{
			pTextTitle->SetText(pstrTitle);
		}
	}

	DuiLib::CDuiString WindowImplBase::GetTitle()
	{
		CTextUI* pTextTitle = (CTextUI*)m_pm.FindControl(_T("title"));
		if (pTextTitle)
		{
			return pTextTitle->GetText();
		}
		return _T("");
	}
	...
	LRESULT WindowImplBase::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		case UIMSG_UPDATE:	m_pm.Invalidate(); break;
